...
# Function for computing PC1 loadings
pc1_loadings = function(s_, genes_, cells_)
{
  liv_mr_mat = t(as.matrix(s_[['RNA']]$data[rownames(s_) %in% genes_, cells_]))                                       # subsetting input sample on malignant cells and LIV-MR signature genes
  liv_mr_mat = liv_mr_mat[,!colSums(liv_mr_mat) == 0]                                                                 # identifying genes with all-zero expression in malignant cells
  
  loadings_ = matrix(0, nrow = 1, ncol = length(genes_), dimnames = list(NULL, genes_))                               # initializing PCA loadings vector
  loadings_[1,colnames(liv_mr_mat)] = prcomp(x = liv_mr_mat, rank. = 2, center = T, scale. = T)$rotation[,'PC1']      # PCA loadings of LIV-MR signature genes in malignant cells
...
preprocess = function(mat_ = NULL, features_ = NULL, barcodes_ = NULL, sample_ = 'sample', DOUBLETS = NULL, min_features = 200, max_features = Inf, max_percent_mt = 10)      # maximum percent of mitochondrial genes in a cell
{
  ...
  # PC1's loadings (weights) ####
  message('Computing PC1 loadings of LIV-MR signature genes in malignant cells')
  
  liv_mr_genes = readLines(con = 'data/LIV-MR_signature.txt')[1:100]      # top 100 LIV-MR signature genes
  loadings_ = pc1_loadings(s_obj, liv_mr_genes, malignant_cells)          # PC1's loadings of LIV-MR signature genes in malignant cells
